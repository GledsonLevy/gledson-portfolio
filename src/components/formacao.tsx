...
export function Formacao() {
  return (
    <section id="formacao" className="py-20 lg:py-32 scroll-mt-10">
            ...
            >
              <div className="md:col-span-3 p-6 md:p-8 md:border-r border-light-muted/20 dark:border-dark-muted/20 flex flex-col justify-between overflow-hidden relative">
                <div className="flex items-center gap-2 mb-8 md:mb-0">
                  <span
                    className={`w-2 h-2 rounded-sm ${isPresent ? "bg-blue-600 dark:bg-blue-500 animate-pulse" : "bg-light-muted dark:bg-dark-muted"}`}
                  ></span>
                  <span
                    className={`font-mono text-xs uppercase tracking-wider ${isPresent ? "text-blue-600 dark:text-blue-400 font-bold" : "text-light-muted dark:text-dark-muted"}`}
                  >
                    {item.type}
                  </span>
                </div>
                ...
                <span
                  className={`text-6xl lg:text-8xl font-black tracking-tighter transition-colors duration-500 ${isPresent ? "text-gray-300 dark:text-slate-800 group-hover:text-blue-500/40 dark:group-hover:text-blue-500/20" : "text-gray-200 dark:text-slate-800/50"}`}
                >
                  {startYear}
                </span>
              </div>
